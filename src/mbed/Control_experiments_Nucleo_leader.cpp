...
char buffer[100];
const char delim[2]="$";
int volatile size = 0;  //size may change from an external factor, e.g pc
int flag = 1;
...
{   
    ...
    esp8266.attach(&esp8266_receive_velocities, Serial::RxIrq); //attach function pointer for receiving
    
    //esp8266.attach(&esp8266_transmitt_volt, Serial::TxIrq);
...
{
    char *tok;
    int tok_num = 0;    //tok_num will help separating and storing data
    velocity vel;
    
    tok=strtok(buffer,delim);   //strtok splits buffer when it reads delim $
    while( tok != NULL ) 
    {
        tok = strtok(NULL, delim);
        if(tok_num == 0)
        {
            vel.right=atof(tok);    //converts tok to floating number
            tok_num++;
        }
        else if(tok_num == 1) {
            vel.left=atof(tok);
